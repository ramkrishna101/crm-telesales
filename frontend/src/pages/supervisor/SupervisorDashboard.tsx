...
import { Users2, Phone, ListChecks, TrendingUp, RefreshCw, Layers3 } from 'lucide-react';
...
export default function SupervisorDashboard() {
  ...
  const connectedCalls = leaderboard.reduce((sum, agent) => sum + agent.connected, 0);
    ...
    <AppLayout>
      <div className="page-container">
        <section className="dashboard-hero">
          <div>
            <p className="section-eyebrow">Team command view</p>
            <h1 className="page-title">Team Overview</h1>
            <p className="page-subtitle">Monitor agent capacity, queue pressure, and live call performance in one place.</p>
          </div>

          <div className="page-actions">
            <div className="ops-pill">{teams.length} teams</div>
          ...
          <div className="metric-ribbon">
            <div className="metric-ribbon__item">
              ...
              <strong className="metric-ribbon__value">{activeAgents}</strong>
              <span className="metric-ribbon__sub">currently available to work</span>
            </div>
            <div className="metric-ribbon__item">
              <span className="metric-ribbon__label">Unassigned queue</span>
              <strong className="metric-ribbon__value">{leadsData?.data?.data?.total || 0}</strong>
              <span className="metric-ribbon__sub">leads waiting for allocation</span>
            </div>
            <div className="metric-ribbon__item">
              <span className="metric-ribbon__label">7 day calls</span>
              <strong className="metric-ribbon__value">{totalCalls}</strong>
              <span className="metric-ribbon__sub">all agents combined</span>
            </div>
            <div className="metric-ribbon__item">
              <span className="metric-ribbon__label">Connect rate</span>
              <strong className="metric-ribbon__value">{connectRate}%</strong>
              <span className="metric-ribbon__sub">connected vs attempted</span>
            </div>
          </div>
        </section>
        ...
        <div className="stats-grid">
          {[
            { label: 'Team Agents', value: agents.length, icon: <Users2 size={20} />, colour: '#6366f1', sub: `${activeAgents} active` },
            ...
            {
              label: 'Connect Rate', value: totalCalls > 0 ? `${connectRate}%` : '—',
        ...
        <div className="card">
          <div className="card-header card-header--dense">
            <div>
              ...
              <h2 className="card-title">Agent Throughput</h2>
            </div>
            <span className="card-subtitle">Total vs connected calls in the last 7 days</span>
          </div>
        ...
        <div className="dashboard-grid">
          <div className="dashboard-stack">
            <div className="card">
              <div className="card-header card-header--dense">
                <div>
                  <div className="card-kicker">Queue management</div>
              ...
              {leads.slice(0, 8).map((l) => (
                <div
                  key={l.id as string}
                  className={`table-row ${selectLead.includes(l.id as string) ? 'table-row--selected' : ''}`}
                  style={{ cursor: 'pointer' }}
                  onClick={() => setSelectLead((selected) => selected.includes(l.id as string) ? selected.filter((id) => id !== l.id) : [...selected, l.id as string])}
                >
                  <div style={{ width: 24, flexShrink: 0 }}>
                    <input type="checkbox" readOnly checked={selectLead.includes(l.id as string)} />
                  </div>
                  <div style={{ flex: 1, marginLeft: 10 }}>
                    <div style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '0.875rem' }}>{(l.name as string) || 'Unknown'}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{l.email as string || ''}</div>
                  </div>
                  <span className="badge" style={{ background: l.priority === 'high' ? '#ef444422' : 'var(--bg-elevated)', color: l.priority === 'high' ? '#ef4444' : 'var(--text-secondary)' }}>
            ...
            <div className="card">
              <div className="card-header card-header--dense">
                <div>
                  <div className="card-kicker">Live feed</div>
              ...
              {recentCalls.slice(0, 8).map((c) => {
                ...
                const tagColour: Record<string, string> = { Interested: '#22c55e', 'Not Interested': '#ef4444', Callback: '#f59e0b', DND: '#dc2626', RNR: '#6366f1', Busy: '#94a3b8' };
                return (
          ...
          <div className="dashboard-stack">
            <div className="card">
              <div className="card-header card-header--dense">
                <div>
                  <div className="card-kicker">Coverage map</div>
                  <h2 className="card-title">Agent Details</h2>
                </div>
                <div className="ops-pill"><Layers3 size={14} /> {leaderboard.length} agents reporting</div>
              </div>
              ...
              {agents.map((a) => {
                const perf = leaderboard.find((leader) => leader.agentId === a.id);
                return (
                  <div key={a.id} className="table-row">
                    ...
                    <div className="table-cell">
                      <span className="badge" style={{ background: a.status === 'active' ? '#14532d' : 'var(--bg-elevated)', color: a.status === 'active' ? '#22c55e' : 'var(--text-secondary)' }}>
                        {a.status}
                      </span>
                    </div>
                    <div className="table-cell" style={{ fontWeight: 700, color: 'var(--text-primary)' }}>{perf?.calls || 0}</div>
